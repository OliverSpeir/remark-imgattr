export default function imgAttr() {
	...
	function parseValue(value) {
			// Remove any unexpected leading and trailing non-standard quotation marks
			value = value.replace(/^“|”$/g, '');

			// Check if the value is wrapped in standard double quotes
			...
			if (node.type === 'paragraph' && node.children && node.children.length) {
					for (let i = 0; i < node.children.length; i++) {
							...
							if (currentNode.type === 'image') {
									...
									if (nextNode && nextNode.type === 'text' && isAttributeString(nextNode.value)) {
											...
											if (node.children[i + 1] && node.children[i + 1].type === 'text' && /^\s*$/.test(node.children[i + 1].value)) {
													node.children.splice(i + 1, 1);
											}
									}
							}
